...
def get_args():
    parser = argparse.ArgumentParser(description="Convert TrueType font (TTF) into dito's internal monospace bitmap font format ('lh4rb').", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def font_to_char_images(font, width_0, max_width, max_height):
    ...
    for char in chars:
        ...
        offset = (0, 0)
        draw.text(offset, "".join(chars + (char,)), font=font, fill="#FFFFFF")
...
def main():
    ...
    for size in args.sizes:
        ...
        out_filename = os.path.join(args.output_dirname, "{}_lh4rb.png".format(max_height))
