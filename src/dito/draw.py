...
def draw_circle(
    image, center, radius, color, thickness, line_type, start_angle=None, end_angle=None
):
    """
...
def draw_regular_polygon(
    image, point_count, position, radius, color, thickness, line_type, angle_offset=0.0
):
    """
    ...
    for angle in np.linspace(
        start=0.0, stop=2.0 * np.pi, num=point_count, endpoint=False
    ):
        points.append(
            [
...
def draw_regular_star(
    ...
    (x, y) = position
    points = []
    for n_point, angle in enumerate(
        np.linspace(start=0.0, stop=2.0 * np.pi, num=2 * point_count, endpoint=False)
    ):
...
def draw_symbol(
    image, symbol, position, radius=4, color=None, thickness=1, line_type=cv2.LINE_AA
):
    """
    ...
    if symbol in ("circle", "o"):
        ...
        cv2.line(
            img=image,
            pt1=dito.core.tir(x - sqrt_one_over_radius, y - sqrt_one_over_radius),
            pt2=dito.core.tir(x + sqrt_one_over_radius, y + sqrt_one_over_radius),
            color=color,
            thickness=thickness,
            lineType=line_type,
        )
        ...
        cv2.line(
            img=image,
            pt1=dito.core.tir(x - radius, y),
            pt2=dito.core.tir(x + radius, y),
            color=color,
            thickness=thickness,
            lineType=line_type,
        )
        cv2.line(
            img=image,
            pt1=dito.core.tir(x, y - radius),
            pt2=dito.core.tir(x, y + radius),
            color=color,
            thickness=thickness,
            lineType=line_type,
        )
        ...
        cv2.rectangle(
            img=image,
            pt1=dito.core.tir(x - radius, y - radius),
            pt2=dito.core.tir(x + radius, y + radius),
            color=color,
            thickness=thickness,
            lineType=line_type,
        )
        ...
        cv2.line(
            img=image,
            pt1=(x, y),
            pt2=dito.core.tir(x + sqrt_05 * radius, y + sqrt_05 * radius),
            color=color,
            thickness=thickness,
            lineType=line_type,
        )
